...
namespace mspfci 
{
    ...
    bool Interface::unpack(Bytes& read_buffer, Bytes& data)
    {
        ...
        else if (read_buffer.back() == 'X')
        {
            ...
            data_size = static_cast<size_t>(static_cast<uint16_t>(read_buffer.back() << 8) | static_cast<uint16_t>(read_buffer.rbegin()[1]));
            code = static_cast<MSPCode>(static_cast<uint16_t>(read_buffer.rbegin()[2] << 8) | static_cast<uint16_t>(read_buffer.rbegin()[3]));
